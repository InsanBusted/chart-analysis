...
@st.cache_data
def load_data(author: str, start_date: date, end_date: date):
    start_str = start_date.strftime("%Y-%m-%d")
    end_str = end_date.strftime("%Y-%m-%d")
    url = f"https://api.ratcs.my.id:5000/api/tiktok/konten/bankData?author={author}&startDate={start_str}&endDate={end_str}"
    res = requests.get(url)
    res.raise_for_status()
    data = res.json()["data"]
...
if author:
    try:
        df = load_data(author, start_date, end_date)
